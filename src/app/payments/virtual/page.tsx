...
export default function VirtualEventTicketingPage() {
    ...
    const checkRegistrationAndFetchEvent = async () => {
      ...
      try {
        ...
        if (!fetchedEvent.isVirtual) {
          console.error("Error: Non-virtual event routed to virtual payment page. EventId:", eventId);
          toast.error("This page is for virtual events. You are being redirected.");
        ...
        if (isFree) {
          toast("This is a free virtual event. Please complete your registration below.");
          // Setup a free ticket if not already handled by general ticket logic
          if (!fetchedEvent.tickets || fetchedEvent.tickets.length === 0) {
            setStandardTicket({
              id: "free_virtual_ticket",
              name: "Free Registration",
              price: 0,
              currency: fetchedEvent.currency || "USD",
              quantity: fetchedEvent.maxAttendees || Infinity,
              sold: 0,
              description: fetchedEvent.shortDescription || fetchedEvent.description || "Access to the virtual event.",
              maxPerOrder: 10 // Default max for free
            });
            setQuantity(1);
          }
        }

        // General ticket setup logic (from original page)
        if (fetchedEvent.tickets && fetchedEvent.tickets.length > 0) {
            ...
            setQuantity(1); 
        } else if (typeof fetchedEvent.price === 'number' && !isFree) { // For paid events with no explicit tickets array
            setStandardTicket({
                id: "synthetic_virtual_id",
                name: "Standard Access",
                price: fetchedEvent.price,
                currency: fetchedEvent.currency || "USD",
                quantity: fetchedEvent.maxAttendees || Infinity,
                sold: 0,
                description: fetchedEvent.shortDescription || fetchedEvent.description || "Full access to the virtual event.",
            });
            setQuantity(1);
        } else if (!isFree) { // If not free and no other ticket info found
             throw new Error("Ticket information for this virtual event is not configured correctly.");
        }

      } catch (error: any) {
        console.error("Error in VirtualEventTicketingPage useEffect:", error);
  ...
  // Pre-fill form with user data if available from session
  useEffect(() => {
    if (session?.user && event) { // Ensure event is loaded before filling
      setFormData(prev => ({
        ...prev,
        email: session.user.email || "",
        // Names might need splitting logic if session.user.name is a full name
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (totalAmount === 0) { // Free virtual event RSVP
        const rsvpData = {
          eventId: eventId,
          ...
          orderType: "FREE_VIRTUAL_EVENT_RSVP" as const
        };
        ...
        router.push(`/payments/success?orderId=${rsvpResult.order?._id || rsvpResult.orderId}&eventId=${eventId}&source=rsvp_virtual`);
      } else { // Paid virtual event
        const backendPaymentPayload = {
          eventId: eventId,
          userId: session.user.id,
          email: formData.email,
          items: [{
            ticketId: standardTicket.id || "standard_virtual_ticket",
          ...
          cancelUrl: `${process.env.NEXT_PUBLIC_BASE_URL}/events/${eventId}`,
          metadata: {
            firstName: formData.firstName,
            lastName: formData.lastName,
            phone: formData.phone,
      ...
      if (ticketingStatus === TicketingStatus.PROCESSING && errorMessage) {
         setTicketingStatus(TicketingStatus.REGISTRATION);
  ...
  if (isLoadingInitialData) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-black to-[#120a19] flex items-center justify-center">
        ...
        <p className="ml-4 text-[#b967ff] animate-pulse">Loading Virtual Event...</p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-black to-[#120a19] flex items-center justify-center">
        ...
        <div className="text-center p-8 rounded-xl shadow-xl bg-[#120a19]/70 backdrop-blur-sm border ${BORDER_ACCENT}">
          ...
          <h2 className="text-2xl font-bold text-white mb-4">Virtual Event Not Found</h2>
          <p className="text-gray-300 mb-6">Details for this virtual event could not be loaded.</p>
  ...
  if (errorMessage && !event) { // If there was an error and event couldn't be loaded
    return (
      <div className="min-h-screen bg-gradient-to-br from-black to-[#120a19] flex items-center justify-center">
        <NoiseBackground />
        <div className="text-center p-8 rounded-xl shadow-xl bg-[#120a19]/70 backdrop-blur-sm border ${BORDER_ACCENT}">
  ...
  // Main Page Content Render for Virtual Event
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-[#120a19] to-black font-sans antialiased">
      ...
      <div className="container mx-auto px-4 py-12">
        {event && (
          <div className="mb-8">
            <Link href={`/events/${eventId}`} className="inline-flex items-center gap-2 text-[#b967ff] hover:text-[#b967ff]/80 transition-colors">
              <ChevronLeft className="w-4 h-4" /> Back to Virtual Event Details
            </Link>
          </div>
        )}
        ...
        {event && standardTicket && (
          <>
            {/* Event Info Section - Simplified for virtual */} 
                  ...
                  {event.coverImage?.url || event.image ? (
                    <div className="relative w-full h-full">
                      <Image 
                        ...
                        alt={event.title || "Virtual event image"}
                        fill
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                      <CalendarIcon className="w-16 h-16 text-[#b967ff]/30" /> {/* Icon for virtual event */} 
                    </div>
                  )}
                </div>
                <div className="md:col-span-2">
                  <h1 className="text-2xl sm:text-3xl font-bold text-white mb-2 leading-tight">{event.title} (Virtual)</h1>
                  <div className="flex items-center text-sm text-gray-400 mb-1">
                    <CalendarIcon className="w-4 h-4 mr-2 text-[#b967ff]" />
                    <span>{formatDate(event.date)}</span>
                  </div>
                  <div className="flex items-center text-sm text-gray-400 mb-3">
                    <Tag className="w-4 h-4 mr-2 text-[#b967ff]" />
